...
function parseVersionAndDate(versionString) {
  if (!versionString) return { version: "missing env: REACT_APP_VERSION", date: "N/A" };

  // Check for format: gwas-target_X.X.X_YYYYMMDD
  const formatMatch = versionString.match(/gwas-target_(\d+\.\d+\.\d+)_(\d{8})/);
  ...
  return (
    <div className="desktop:grid-col-4">
      <div className="usa-footer__primary-content usa-footer__primary-content--collapsible">
        <span className="usa-footer__primary-link usa-footer__nci-list-header">{title}</span>
        <ul className="usa-list--unstyled">
          {links.map(({ label, href, id, isText }) =>
            isText ? (
              <li key={label} className="usa-footer__secondary-link">
                {label}
              </li>
            ) : (
              <li key={label} className="usa-footer__secondary-link">
                <a id={id} href={href} target="_blank" rel="noopener noreferrer" className="fw-normal">
                  {label}
                </a>
              </li>
            )
...
function BackToTop({ onClick }) {
  return (
    <div className="usa-footer__return-to-top">
      <button type="button" aria-label="Back To Top" id="backToTop" onClick={onClick}>
...
export default function Footer() {
  ...
  const isBackToTopVisible = useScrollVisibility(100);

  const handleBackToTop = () => window.scrollTo({ top: 0, behavior: "smooth" });

  const aboutLinks = [
    { label: "About GWAS Target", href: "https://analysistools.cancer.gov/gwas-target" },
    { label: "Contact Us", href: "mailto:NCIGWASTargetWebAdmin@mail.nih.gov" },
    {},
    { label: `Last Updated: ${date}`, isText: true },
    { label: `Version: ${version}`, isText: true },
  ];
  ...
  const policyLinks = [
    ...
    { label: "Vulnerability Disclosure", href: "https://www.hhs.gov/vulnerability-disclosure-policy/index.html" },
  ];
  ...
  return (
    <footer className="usa-footer usa-footer--nci-big flex-grow-0">
      ...
      {isBackToTopVisible && <BackToTop onClick={handleBackToTop} />}
    </footer>
  );
}
